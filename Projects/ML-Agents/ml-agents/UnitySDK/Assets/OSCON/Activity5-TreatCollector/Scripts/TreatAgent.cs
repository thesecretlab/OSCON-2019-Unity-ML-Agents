...
public class TreatAgent : Agent
{
    private TreatAcademy myAcademy;
    ...
    public override void InitializeAgent()
    {
        ...
        myArea = area.GetComponent<TreatArea>();
        rayPer = GetComponent<RayPerception3D>();
        myAcademy = FindObjectOfType<TreatAcademy>();
    }
    ...
    public override void CollectObservations()
    {
        if (useVectorObs)
        {
            ...
            string[] detectableObjects = { "badTreat", "agent", "wall", "goodTreat", "frozenAgent" };
    ...
    public void MoveAgent(float[] act)
    {
        ...
        if (shoot)
        {
            ...
            if (Physics.SphereCast(transform.position, 2f, position, out hit, 25f))
            {
                ...
                {
                    hit.collider.gameObject.GetComponent<TreatAgent>().Freeze();
                }
            }
        }
        else
    ...
    {
        if (collision.gameObject.CompareTag("goodTreat"))
        {
            Satiate();
            collision.gameObject.GetComponent<TreatLogic>().OnEaten();
            AddReward(1f);
            treats += 1;
            if (contribute)
            {
        ...
        if (collision.gameObject.CompareTag("badTreat"))
        {
            Poison();
            collision.gameObject.GetComponent<TreatLogic>().OnEaten();
